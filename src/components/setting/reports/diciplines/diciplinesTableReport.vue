...
const handleReports = async (data: object = {}) => {
  loading.value = true
  try {
    const response = await getDiciplineReports(data)
...
const handleDownload = async (data: object = {}) => {
  loading.value = true
  try {
    const response = await downloadDisciplinesReports({
      ...data,
        ...
        <table class="table is-hoverable is-fullwidth">
          ...
          <tbody v-if="reports && !loading">
            <tr v-for="(report, key) in reports" :key="`report-${key}`">
              ...
              <td>
                {{
                  `${report.name} ${report.second_name ?? ''} ${
                    report.last_name
                  }`
                }}
              </td>
