...
const data = computed(() => {
  let data = props.subscriptions

  // data.map((s) => {
  //   if (s.user) {
  //     let suscripcion = props.suscripciones.find((e) => e.user_id == s.user.id)
  //     if (suscripcion != undefined) {
  //       s.suscripcion = suscripcion
  //     }
  //   }
  //   return s
  // })
...
const closeModal = () => {
  suscripcion.modal = false
}

const viewSuscripcion = (item) => {
  console.log('viewSuscripcion', item.suscripcion_id)
  if (item.suscripcion_id) {
    suscripcion.suscripcion_id = item.suscripcion_id
  }

  suscripcion.modal = true
}
</script>
...
<template>
  <div>
    <table class="table is-hoverable is-fullwidth">
      ...
      <tbody :key="`subs-${key}`" v-for="(item, key) in data">
        <tr :id="`subs-${key}`" :class="key == keySelected ? 'bg-info' : ''">
          ...
          <td>
            ...
            <VButton color="success" outlined @click="viewSuscripcion(item)">
                        ...
                        <table class="table is-hoverable is-fullwidth">
                          ...
                          <tbody>
                            <tr>
                              ...
                              <td>
                                <p>
                                  {{ item_.payment_intent_data.description }}
                                </p>
                              </td>
                              <td>
                              ...
                              <td>
                                <p>
                                  {{
                                    moneda(
                                      item_.payment_intent_data.amount / 100
                                    )
                                  }}
                                </p>
                              </td>
    ...
    >
      <template #content>
        <!-- <p>{{ suscripcion }}</p> -->
        <subscriptionIndex
          v-if="suscripcion.modal && suscripcion.suscripcion_id"
          :suscripcion_id="suscripcion.suscripcion_id"
        />
        <p v-else>Not subscription</p>
      </template>
      <template #action>
        <!-- <VButton
          :to="{
