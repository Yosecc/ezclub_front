...
import {
  ...
  isLoading,
  direccionInput,
} from '/@src/state/wizardStateLandingMembersUpdated.ts'

onMounted(async () => {
  isLoading.value = true
  await getCompanySlug(route.query.company)
    .then((response) => {
      company.value.logo = `${API_WEB_URL.value}storage/${response.data.logo}`
      company.value.name = response.data.company_name
...
<template>
  <LayoutLanding>
    <!-- <p>{{ companyData }}</p> -->
    <WizardLanginNavigation v-model:step="currentStep" :title="stepTitle" />
    ...
    <div class="wizard-v1-wrapper">
      <div class="pt-2 container">
        <div class="m-2 d-flex justify-content-center">
          <InformationStep1 v-if="currentStep == 1" />
          <InformationStep2 v-if="currentStep == 2" />
          <InformationStep3 v-if="currentStep == 3" />
          <InformationStep4 v-if="currentStep == 4" />
        </div>
      </div>
    </div>
  </LayoutLanding>
